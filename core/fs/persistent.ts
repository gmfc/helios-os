...
export class PersistentFileSystem implements AsyncFileSystem {
    ...
    static async load(): Promise<PersistentFileSystem> {
        ...
        const root = await db.select<Inode[]>(
    ...
    private async lookup(path: string): Promise<Inode | undefined> {
        if (path === "/") {
            const res = await this.db.select<Inode[]>(
                "SELECT * FROM inodes WHERE id=1",
            );
            const inode = res[0];
    ...
    private async initDefaultFiles(): Promise<void> {
        ...
        await this.createFileInternal(
            "/etc/issue",
            0o644,
            enc("Welcome to Helios-OS v0.1\n"),
        );
        ...
        await this.createFileInternal(
            "/bin/browser.manifest.json",
            0o644,
            enc(BROWSER_MANIFEST),
        );
        await this.createFileInternal("/bin/ping", 0o755, enc(PING_SOURCE));
        await this.createFileInternal(
            "/bin/ping.manifest.json",
            0o644,
            enc(PING_MANIFEST),
        );
        await this.createFileInternal(
    ...
    async open(path: string, flags = "r"): Promise<FileSystemNode | undefined> {
        ...
        const ts = Date.now();
        await this.db.execute("UPDATE inodes SET atime=?1 WHERE id=?2", [
            ts,
            inode.id,
        ]);
        inode.atime = ts;
    ...
    async read(path: string): Promise<Uint8Array> {
        ...
        if (!inode || inode.kind !== "file") {
            throw new Error(
                `ENOENT: no such file or directory, read '${path}'`,
            );
        }
        const rows = await this.db.select<{ blob: Uint8Array }[]>(
            "SELECT blob FROM file_data WHERE inode_id=?1",
            [inode.id],
        );
    ...
    async write(path: string, data: Uint8Array): Promise<void> {
        ...
        if (!inode || inode.kind !== "file") {
            throw new Error(
                `ENOENT: no such file or directory, write '${path}'`,
            );
        }
        const ts = Date.now();
        await this.writeTx(async () => {
            await this.db.execute(
                "REPLACE INTO file_data (inode_id, blob) VALUES (?1, ?2)",
                [inode.id, data],
            );
            await this.db.execute(
                "UPDATE inodes SET size=?1, mtime=?2, atime=?2 WHERE id=?3",
                [data.length, ts, inode.id],
            );
        });
    ...
    async readdir(path: string): Promise<FileSystemNode[]> {
        ...
        return rows.map((r) =>
            this.toNode(path + (path === "/" ? "" : "/") + r.name, r),
        );
    }
    ...
    async mount(image: FileSystemSnapshot, path: string): Promise<void> {
        ...
        if (!mount) {
            ...
            throw new Error(
                `ENOTDIR: mount point is not a directory, mount '${path}'`,
            );
        }
        ...
        for (const node of entries) {
            ...
            if (await this.lookup(newPath)) {
                throw new Error(
                    `EEXIST: file already exists, mount '${newPath}'`,
                );
            }
            if (node.kind === "dir") {
